...
### SHP wiring and the first real coverage numbers (2026-08-08)

Issue #29 is fixed: `sequential.rs` now builds the configured SHP mask from each ministack's
real acquisitions and passes it to the covariance kernel. `shp_method: rect` keeps the
unmasked kernel and is bit-identical to the pre-SHP output, so every dolphin-oracle contract
(which was generated by `run_phase_linking` **without** `neighbor_arrays`) keeps its meaning;
those fixture configs now say `shp_method: rect` explicitly instead of inheriting dolphin's
GLRT config default. `tests/shp_wiring_contract.rs` pins both directions on a synthetic
window straddling two scattering populations.

The MMX1/ICMX matrix now scores to completion (`status: pass`, exit 0) and emits
`uncertainty_reliability.{json,csv,svg}`. Measured effects:

- finite footprint **98.8613% → 99.3062%** (singular pixels 1.1387% → 0.6938%, a 39%
  reduction). This is smaller than the 82.5% rescue rate measured on the old singular set,
  because the mask also perturbs pixels that were previously well conditioned.
- ICMX station windows **0/1, 0/9, 5/25, 14/49 → 1/1, 9/9, 24/25, 44/49**; MMX1 stays fully
  finite. Both stations now clear the 50% contract.
- velocity: InSAR **−251.143 ± 13.553 mm/yr** against GNSS **−247.811 mm/yr**, difference
  **−3.332 mm/yr** (1.3%), identical across all four engines.

Empirical coverage of the MMX1−ICMX differential residual, 12 evaluated epochs:

| engine | method | 68% | 90% | 95% |
|---|---|---:|---:|---:|
| native / snaphu (weighted) | CRLB-only | 0.500 | 0.583 | 0.833 |
| native / snaphu (weighted) | posterior-only | 0.500 | 0.583 | 0.833 |
| native / snaphu (weighted) | quadrature | 0.500 | 0.667 | 0.833 |
| unweighted | CRLB-only | 0.500 | 0.583 | 0.833 |
| unweighted | posterior-only | 0.583 | 0.917 | 1.000 |
| unweighted | quadrature | 0.583 | 0.917 | 1.000 |

**These are indicative, not a calibration claim.** Twelve temporally correlated samples
quantize every bin at 8.3%, so one epoch moves a number by a full step, and the differential
residual is one station pair on one burst. Read as a direction rather than a number, the
