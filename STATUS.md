...
| 2b. End-to-end NRT front door (branch `v1.4-nrt-frontdoor`) | ✅ — `run_displacement_resumable` + `update_displacement` carry per-burst `SequentialState` through the whole pipeline; downstream (network→unwrap→timeseries→velocity) recomputes from the updated phase history. **End-to-end incremental update bit-identical to a full `run_displacement`** (max\|Δ\| = 0 through SNAPHU + SBAS; `nrt_displacement_contract.rs`, ministack_size=2 to seal mid-update) + a no-op guard. `dolphin stream --config --initial N` CLI subcommand (verified on the disp fixture, series grows 1→4 bands). Constraint: an update extends every burst (a pass yields one CSLC/burst). Gates green (default + `no-gpu`); sign guard green. **Awaiting sign-off to merge.** |
...
| 4. Phase-bias / non-closure correction (branch `v1.4-phasebias`) | ✅ — `dolphin-phaselink::phasebias` (Michaelides 2022, leads the oracle). First-order constant-bias-velocity model from the v1.2 closure layer; opt-in `phase_linking.correct_phase_bias` (off by default → default output + oracle/sign contracts unchanged). Analytic contract exact (constant bias recovered <1e-9); **non-closure 0.800→0.095 rad (8.4×)** on a 100-date series; wired into the pipeline + e2e test. Gates green (default + `no-gpu`). Merged. |
| 5. 3D-unwrap-ready dispatch interface (branch `v1.4-unwrap-iface`) | ✅ — `unwrap_backend::UnwrapBackend`, a network-level trait (linked phase + date pairs) so a spurt-style 3D solver drops in as a new backend with no pipeline change. `SnaphuBackend` + `TophuBackend` implement it via the unchanged per-ifg loop; **output bit-identical** (end-to-end oracle contract still passes through the new dispatch; trait seam test `unwrap_backend_contract.rs`). No spurt port. Gates green (default + `no-gpu`). Merged. |
